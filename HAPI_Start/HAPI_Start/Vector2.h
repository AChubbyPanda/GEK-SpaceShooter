...
public:
	...
	//add a SquaredLength fn that returns the squared length fir use in comparisons etc:
	float Length() const
	{
		return sqrt(x * x + y * y);
	}
	...
	If both vectors are already unit vectors then the dot produt returns the
	consine of the angle between them. If they are not unit vectors then you must divide
	the result by the product of the vector lengths to get the cosine of the angle.
	Another use of the dor prouct is to find the distance of a point to a plane
	*/
	...
	/*
	The cross produxt gives a value which is 2 times the area of the triangle.
